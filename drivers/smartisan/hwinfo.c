...
//Smartisan BSP gaosc add for ocean no nfc end
	...
	return sprintf(hwinfo[version_id].hwinfo_buf, "%s:%s:0x%x",
		product_version_table[product_version_id].product_version,
		hw_version_table[hw_version_id].hw_version,
		product_version_id << 4 | hw_version_id);
}
